...
const resolvers = {
    Query: {
        async getProduct(_, args, contextValue) { // Apollo Server has 4 parameters-> (parent, args, contextValue, info)
            ...
            const bucket : couchbase.Bucket = contextValue.couchbaseCluster.bucket('store-bucket');
            const collection : couchbase.Collection = bucket.scope('products-scope').collection('products');

            const getResult : couchbase.GetResult = await collection.get(id).catch( (error) => {
                console.log(error);
    ...
    Mutation: {
        async createProduct(_, args, contextValue) { 
            ...
            const createdMutationResult : couchbase.MutationResult = await collection.insert(key, product).catch( (error) => {
                console.log(error);
                throw error; // "Document not found"
            });
        ...
        async deleteProduct(_, args, contextValue) { 
            ...
            const deletedMutationResult : couchbase.MutationResult = await collection.remove(id).catch( (error) => {
                console.log(error);
                throw error; // "Document not found"
            });
        ...
        async updateProduct(_, args, contextValue) { 
            ...
            const updatedMutationResult : couchbase.MutationResult = await collection.replace(id, product).catch( (error) => {
                console.log(error);
                throw error; // "Document not found"
            });
        ...
        async setQuantity(_, args, contextValue) { 
            ...
            const updatedMutationResult : couchbase.MutationResult = await collection.mutateIn(id, 
                [
